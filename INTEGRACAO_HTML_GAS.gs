...
const COL_LINKS = {
  apto: 1,                    // A
  inquilino: 2,               // B
  linhaFormResponse: 3,       // C
  linkPastaContrato: 4,       // D
  linkFoto3x4: 5,             // E
  linkDocumentoRG: 6,         // F
  linkContratoPDF: 7,         // G
  linkContratoAssinado: 8,    // H
  linkDeclaracao: 9           // I
};
...
function normalizarNomeArquivo(texto) {
  ...
  return texto.toString().trim().normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
function detectarGenero(estadoCivil) {
  ...
  if (texto.includes('solteira') || texto.includes('casada') || texto.includes('divorciada') || texto.includes('viúva')) {
    return 'a';
  }
  if (texto.includes('solteiro') || texto.includes('casado') || texto.includes('divorciado') || texto.includes('viúvo')) {
    return 'o';
  }
  return 'o(a)';
}
...
function menuResumo() {
  ...
  ui.alert(
    ...
    'Total de apartamentos: ' + (dados.length - 1),
    ui.ButtonSet.OK
  );
}
...
// -----------------------------------------------------
// MENUS (chamados pelo BASE250_Menu.gs)
// -----------------------------------------------------
